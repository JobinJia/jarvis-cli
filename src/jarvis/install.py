...
Commands:
  jarvis install     - write config, hook into ~/.claude/settings.json, install plist
  jarvis uninstall   - reverse install steps (keeps user data)
  jarvis status      - check daemon health
  jarvis test        - send a synthetic event to the daemon
"""
...
_CODEX_BLOCK_START = "# === jarvis:start ==="
_CODEX_BLOCK_END = "# === jarvis:end ==="
...
def _is_our_hook(hook: dict) -> bool:
    """Match any entry whose command basename is jarvis-hook (legacy bare
    name or absolute path from a previous install in a different venv)."""
    return Path(hook.get("command", "")).name == "jarvis-hook"
...
def merge_claude_settings(existing: dict, hook_command: str) -> dict:
    ...
    PostToolUse, replacing any prior jarvis-hook entries in those buckets.
    """
...
def remove_from_claude_settings(existing: dict, hook_command: str) -> dict:
    """Strip our jarvis-hook entries from every hook bucket we install into.
...
# --- install wizard --------------------------------------------------------
#
# Goal: a first-time `jarvis install` should leave a working, opinionated
...
def _render_configured_toml(choices: WizardChoices, *, preserve: dict | None = None) -> str:
    ...
    return textwrap.dedent(f"""\
        # jarvis config.toml — written by `jarvis install` wizard.
        # Re-run with `jarvis install --reconfigure` to walk through again.
        [llm]
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    if settings_path.exists():
        try:
            ...
            return 2
    hook_command = shutil.which("jarvis-hook")
    if not hook_command:
        print("!! jarvis-hook not on PATH — did you run `uv sync`?", file=sys.stderr)
        return 4
    ...
    # 4. Write plist
    program = shutil.which("jarvis-daemon")
    if not program:
        print("!! jarvis-daemon not on PATH — did you run `uv sync`?", file=sys.stderr)
        return 3
    ...
    print(
        ...
        f"     set it now and re-run `jarvis install` (this re-bakes the plist).\n"
...
def cmd_uninstall(args: argparse.Namespace) -> int:
    ...
    if settings_path.exists():
        try:
            existing = json.loads(settings_path.read_text())
            pruned = remove_from_claude_settings(existing, hook_command="jarvis-hook")
...
def cmd_tone(args: argparse.Namespace) -> int:
    ...
    if args.level is None:
        ...
        print("set with: jarvis tone <0-3>")
        return 0
    ...
    if n == 0:
        print(
            ...
            "`jarvis install --reconfigure` first",
            file=sys.stderr,
        )
        return 1
...
def main() -> int:
    ...
    p_un.add_argument("--purge", action="store_true", help="also remove ~/.jarvis/")
